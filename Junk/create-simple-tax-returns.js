...
async function createUsersAndTaxReturns() {
  try {
    ...
    // First, delete existing data if it exists (delete in correct order due to foreign keys)
    for (const userData of users) {
      console.log(`Cleaning up existing data for ${userData.name}...`);

      // Delete all related tax return data first
      const userResult = await pool.query('SELECT id FROM users WHERE email = $1', [userData.email]);
      if (userResult.rows.length > 0) {
        const userId = userResult.rows[0].id;

        // Delete in order: forms first, then tax_returns, audit_log, then users
        await pool.query('DELETE FROM wealth_forms WHERE user_id = $1', [userId]);
        await pool.query('DELETE FROM capital_gain_forms WHERE user_id = $1', [userId]);
        await pool.query('DELETE FROM deductions_forms WHERE user_id = $1', [userId]);
        await pool.query('DELETE FROM credits_forms WHERE user_id = $1', [userId]);
        await pool.query('DELETE FROM adjustable_tax_forms WHERE user_id = $1', [userId]);
        await pool.query('DELETE FROM income_forms WHERE user_id = $1', [userId]);
        await pool.query('DELETE FROM tax_returns WHERE user_id = $1', [userId]);
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      // 2. Create tax return
      const returnNumber = `TR-${TAX_YEAR}-${user.name.replace(/\s+/g, '').toUpperCase()}-${Date.now() + i}`;
      ...
      `, [
        ...
        adjustableTaxData.profit_debt_tax
      ]);
      ...
      `, [
        taxReturnId, user.id, user.email, TAX_YEAR_ID, TAX_YEAR,
        creditsData.charity, creditsData.pension, creditsData.insurance
      ]);
      ...
      `, [
        taxReturnId, user.id, user.email, TAX_YEAR_ID, TAX_YEAR,
        deductionsData.professional, deductionsData.zakat, deductionsData.advance
      ]);
      ...
      `, [
        ...
        capitalGainsData.securities, capitalGainsData.other
      ]);
...
function getAdjustableTaxData(profile) {
  switch(profile) {
    case 'HIGH_EARNER':
      return {
        salary_tax: 180000,
        electricity_tax: 45000,
        phone_tax: 15000,
        vehicle_tax: 2400000,
        profit_debt_tax: 120000,
        total: 2760000
      };
    case 'MEDIUM_EARNER':
      return {
