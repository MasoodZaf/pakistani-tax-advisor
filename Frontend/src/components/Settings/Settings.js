    ...
    <div className="bg-white rounded-lg shadow-sm p-6 dark:bg-[#151c30]">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          <Activity className="w-6 h-6 text-gray-600 dark:text-[#aab2cc]" />
          <h2 className="text-lg font-semibold text-gray-900 dark:text-[#e7eaf3]">Recent Activity</h2>
        </div>
        <button
          onClick={load}
          disabled={loading}
          className="inline-flex items-center gap-1 text-xs font-semibold text-gray-600 hover:text-gray-900 dark:text-[#aab2cc] dark:hover:text-[#e7eaf3]"
          title="Reload"
        >
      ...
      {error && (
        <p className="text-sm text-red-600 mb-2 dark:text-red-400">Couldn&apos;t load activity: {error}</p>
      )}

      {!loading && items.length === 0 && !error && (
        <p className="text-sm text-gray-500 dark:text-[#7e88a6]">No activity recorded yet.</p>
      )}

      {visible.length > 0 && (
        <ul className="divide-y divide-gray-100 dark:divide-[#2a3450]">
          {visible.map((it) => (
            <li key={it.id} className="py-3 flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-gray-900 dark:text-[#e7eaf3]">
                  {ACTIVITY_LABELS[it.action] || it.action}
                </p>
                {(it.change_summary || it.field_name) && (
                  <p className="text-xs text-gray-600 mt-0.5 dark:text-[#aab2cc]">
                    {it.change_summary || `Updated ${it.field_name}`}
                  </p>
                )}
                {it.ip_address && (
                  <p className="text-[11px] text-gray-400 mt-0.5 dark:text-[#7e88a6]">
                    From {it.ip_address}
                  </p>
                )}
              </div>
              <time
                className="text-xs text-gray-500 whitespace-nowrap dark:text-[#7e88a6]"
      ...
      {items.length > 10 && (
        <button
          ...
          className="mt-3 text-sm font-semibold text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300"
        >
...
const WizardRerunControl = () => {
  ...
  if (!status) {
    return <p className="text-sm text-gray-500 dark:text-[#7e88a6]">Loading…</p>;
  }
    ...
    return (
      <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg dark:border-[#2a3450]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-medium text-gray-900 dark:text-[#e7eaf3]">Wizard is in progress</div>
            <div className="text-sm text-gray-600 dark:text-[#aab2cc]">Resume where you left off.</div>
          </div>
        </div>
        <button
          type="button"
  ...
  if (!status.completed) {
    return (
      <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg dark:border-[#2a3450]">
        <div className="flex items-center gap-3">
          <Sparkles className="w-5 h-5 text-gray-500 dark:text-[#7e88a6]" />
          <div>
            <div className="font-medium text-gray-900 dark:text-[#e7eaf3]">You haven't run the wizard yet</div>
    ...
    <div>
      <div className="flex items-center justify-between p-3 border border-gray-200 rounded-lg dark:border-[#2a3450]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-medium text-gray-900 dark:text-[#e7eaf3]">Wizard completed for this year</div>
            <div className="text-sm text-gray-600 dark:text-[#aab2cc]">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-md border border-gray-300 hover:bg-gray-50 disabled:opacity-50 dark:border-[#2a3450] dark:hover:bg-[#1a2238] dark:text-[#e7eaf3]"
        >
      ...
      {confirming && (
        <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm dark:bg-amber-500/15 dark:border-amber-500/30">
          <p className="text-amber-900 font-medium mb-2 dark:text-amber-300">Run the wizard again?</p>
            ...
            {confirming ? (
              <div className="flex items-center gap-2">
                <button
                  onClick={deregister}
                  disabled={busy}
                  className="px-4 py-2.5 text-sm font-semibold rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors disabled:opacity-50"
                >
                  {busy ? 'Removing…' : 'Yes, deregister'}
                </button>
                <button
                  onClick={() => setConfirming(false)}
                  className="px-4 py-2.5 text-sm font-semibold rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors dark:border-[#2a3450] dark:text-[#aab2cc] dark:hover:bg-[#1a2238]"
                >
                  Cancel
                </button>
              </div>
              ...
              <button
                onClick={() => setConfirming(true)}
                className="px-4 py-2.5 text-sm font-semibold rounded-lg border border-red-200 text-red-600 hover:bg-red-50 transition-colors dark:border-red-900 dark:text-red-300 dark:hover:bg-red-950"
              >
                Deregister
              </button>
            )}
...
const Settings = () => {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="bg-white rounded-lg shadow-sm p-6 dark:bg-[#151c30]">
        <h1 className="text-2xl font-bold text-gray-900 mb-2 dark:text-[#e7eaf3]">Account Settings</h1>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 dark:bg-[#151c30]">
        <div className="flex items-center space-x-3 mb-6">
          <User className="w-6 h-6 text-gray-600 dark:text-[#aab2cc]" />
          <h2 className="text-lg font-semibold text-gray-900 dark:text-[#e7eaf3]">Profile Information</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-[#aab2cc]">Full Name</label>
            <input
              type="text"
              value={user?.name || ''}
              disabled
              className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 dark:border-[#2a3450] dark:bg-[#0f1426] dark:text-[#e7eaf3]"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-[#aab2cc]">Email Address</label>
            <input
              type="email"
              value={user?.email || ''}
              disabled
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-[#aab2cc]">CNIC</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2 dark:text-[#aab2cc]">Role</label>
            <input
              type="text"
              value={user?.role?.replace('_', ' ') || ''}
              disabled
              className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 capitalize dark:border-[#2a3450] dark:bg-[#0f1426] dark:text-[#e7eaf3]"
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 dark:bg-[#151c30]">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center space-x-3">
            <SlidersHorizontal className="w-6 h-6 text-gray-600 dark:text-[#aab2cc]" />
            <h2 className="text-lg font-semibold text-gray-900 dark:text-[#e7eaf3]">Income Streams</h2>
          </div>
          {streamsDirty && (
            <div className="flex items-center gap-2">
              <button
                onClick={cancelStreams}
                className="px-3 py-1.5 text-sm font-medium text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors dark:text-[#aab2cc] dark:bg-[#1a2238] dark:hover:bg-[#2a3450]"
              >
                Cancel
              </button>
              <button
        ...
        <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-lg px-4 py-3 mb-4 dark:bg-green-500/15 dark:border-green-500/30">
          <span className="text-lg">💼</span>
          <div className="flex-1">
            <p className="text-sm font-700 font-semibold text-green-900 dark:text-green-300">Salaried Employee</p>
            <p className="text-xs text-green-700 dark:text-green-400">Primary income type — always included</p>
          </div>
        ...
        <p className="text-sm text-gray-500 mb-3 dark:text-[#7e88a6]">
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-900 truncate dark:text-[#e7eaf3]">{stream.title}</p>
                  <p className="text-xs text-gray-500 leading-tight line-clamp-1 dark:text-[#7e88a6]">{stream.desc}</p>
                </div>
                <div
        ...
        <p className="text-xs text-gray-400 mt-3 dark:text-[#7e88a6]">
      ...
      {removingStream && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-sm mx-4 shadow-xl dark:bg-[#151c30]">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 bg-amber-100 rounded-full flex items-center justify-center flex-shrink-0 dark:bg-amber-500/15">
                  <AlertTriangle className="w-5 h-5 text-amber-600 dark:text-amber-400" />
                </div>
                <h3 className="text-base font-semibold text-gray-900 dark:text-[#e7eaf3]">Remove income stream?</h3>
              </div>
              <p className="text-sm text-gray-600 mb-1 dark:text-[#aab2cc]">
                <strong>{stream?.icon} {stream?.title}</strong> will be removed from your filing flow.
              </p>
              <p className="text-sm text-gray-500 mb-5 dark:text-[#7e88a6]">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 dark:text-[#aab2cc] dark:bg-[#1a2238] dark:hover:bg-[#2a3450]"
                >
                  Keep it
                </button>
                <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 dark:bg-[#151c30]">
          <div className="flex items-center space-x-3 mb-6">
            <Bell className="w-6 h-6 text-gray-600 dark:text-[#aab2cc]" />
            <h2 className="text-lg font-semibold text-gray-900 dark:text-[#e7eaf3]">Notifications</h2>
          </div>

          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-medium text-gray-900 dark:text-[#e7eaf3]">Email Notifications</div>
                <div className="text-sm text-gray-600 dark:text-[#aab2cc]">Receive updates via email</div>
              </div>
              <button className="relative inline-flex h-6 w-11 items-center rounded-full bg-gray-200 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:bg-[#1a2238]">
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="font-medium text-gray-900 dark:text-[#e7eaf3]">Deadline Reminders</div>
                <div className="text-sm text-gray-600 dark:text-[#aab2cc]">Get reminders for tax deadlines</div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 dark:bg-[#151c30]">
          <div className="flex items-center space-x-3 mb-6">
            <Shield className="w-6 h-6 text-gray-600 dark:text-[#aab2cc]" />
            <h2 className="text-lg font-semibold text-gray-900 dark:text-[#e7eaf3]">Security</h2>
          </div>

          <div className="space-y-6">
            <button
              onClick={() => setShowPasswordModal(true)}
              className="w-full text-left p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors dark:border-[#2a3450] dark:hover:bg-[#1a2238]"
            >
              <div className="font-medium text-gray-900 dark:text-[#e7eaf3]">Change Password</div>
              <div className="text-sm text-gray-600 dark:text-[#aab2cc]">Update your account password</div>
            </button>
            ...
            <div>
              <h3 className="font-medium text-gray-900 mb-3 dark:text-[#e7eaf3]">Connected accounts</h3>
              <ConnectedAccounts />
            </div>
            ...
            <div>
              <h3 className="font-medium text-gray-900 mb-3 dark:text-[#e7eaf3]">Quick-start wizard</h3>
              <WizardRerunControl />
            </div>
          </div>
        </div>
      ...
      {/* Help & Support */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 dark:bg-blue-500/15 dark:border-blue-500/30">
        <div className="flex items-center space-x-3 mb-4">
          <HelpCircle className="w-6 h-6 text-blue-600 dark:text-blue-400" />
          <h3 className="font-semibold text-blue-900 dark:text-blue-300">Need Help?</h3>
        </div>
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4 dark:bg-[#151c30]">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-[#e7eaf3]">Change Password</h3>
              <button
                onClick={() => setShowPasswordModal(false)}
                className="text-gray-400 hover:text-gray-600 dark:text-[#7e88a6] dark:hover:text-[#aab2cc]"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handlePasswordChange} className="space-y-4">
              {/* Current Password */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 dark:text-[#aab2cc]">
                  Current Password
                </label>
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent pr-10 dark:border-[#2a3450] dark:bg-[#0f1426] dark:text-[#e7eaf3]"
                    required
                  />
                  <button
                    type="button"
                    onClick={() => togglePasswordVisibility('current')}
                    className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600 dark:text-[#7e88a6] dark:hover:text-[#aab2cc]"
                  >
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1 dark:text-[#7e88a6]">Must be at least 8 characters long</p>
              </div>
              ...
              <div className="flex space-x-3 pt-4">
                <button
                  type="button"
                  onClick={() => setShowPasswordModal(false)}
                  className="flex-1 px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 dark:text-[#aab2cc] dark:bg-[#1a2238] dark:hover:bg-[#2a3450]"
