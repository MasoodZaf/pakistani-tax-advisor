...
async function taxOptimization({ userId, taxYear, includePII = false, taxData }) {
  const q = [
    ...
    `- If a relief IS present in claimedReliefs, do NOT tell them to "claim" it. Only mention it if there is statutory HEADROOM to claim MORE — phrase it as "you've claimed PKR X; the cap allows up to PKR Y, so you could add PKR Z". If there's no headroom, omit it.`,
    ``,
    `PRECISE NUMBERS — "reliefHeadroom" (when present) is AUTHORITATIVE, computed server-side from the DB caps and the exact average tax rate. For the capped credits it lists, you MUST use its figures verbatim:`,
    `- estimatedSavingPKR = maxAdditionalCreditPKR for that relief (do NOT re-estimate it).`,
    `- In the action, quote additionalEligiblePKR ("you can contribute up to PKR <additionalEligiblePKR> more") and alreadyClaimedPKR.`,
    `- If additionalEligiblePKR is 0 for a relief, that relief is maxed out — omit it.`,
    ``,
